...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Regulations GOV');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!apiKey) {
    throw new Error(
      'Regulations.gov requires a data.gov API key: pass your key as the _apiKey argument (free at https://open.gsa.gov/api/regulationsgov/).',
    );
  }
  switch (name) {
    case 'search_dockets':
